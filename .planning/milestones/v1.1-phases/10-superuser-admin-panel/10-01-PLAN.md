...
3. **Admin router** (`packages/api/src/routers/admin.ts`):
   - Create new router file with initial procedures (all using `adminProcedure`):
     ...
     - `getUsers` — input: `{ search?: string, page?: number, limit?: number }`, returns paginated UserProfile list with totalCount. Search by name AND email:
       - Name search: standard Prisma `contains` filter on UserProfile.name
       - Email search: use Supabase admin client (`createClient({ serviceRole })`) to call `supabase.auth.admin.listUsers()`, filter by email match, collect matching user IDs, then OR-combine with name filter in Prisma query: `WHERE name CONTAINS search OR id IN (matchedAuthUserIds)`
       - This satisfies ROADMAP Success Criteria #3: "Админ может найти пользователя по email"
       - Include `_count` of diagnosticSessions per user
