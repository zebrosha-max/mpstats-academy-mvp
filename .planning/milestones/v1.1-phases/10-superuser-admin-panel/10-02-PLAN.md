...
must_haves:
  truths:
    ...
    - "Admin can search users by name or email and see paginated list"
  ...
  key_links:
    ...
    - from: "apps/web/src/app/(admin)/admin/analytics/page.tsx"
      to: "packages/api/src/routers/admin.ts"
      via: "trpc.admin.getAnalytics.useQuery()"
      pattern: "trpc\\.admin\\.getAnalytics"
...
3. **UserTable component** (`apps/web/src/components/admin/UserTable.tsx`):
   ...
   - Search input at top: debounced (300ms) text input filtering by name or email (backend handles both via Prisma + Supabase auth admin API)
