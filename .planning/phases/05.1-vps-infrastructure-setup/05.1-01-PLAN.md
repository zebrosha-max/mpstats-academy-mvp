...
must_haves:
  truths:
    ...
    - "Docker и Docker Compose установлены и работают на VPS"
    - "Nginx установлен и запущен на VPS"
    - "UFW активен с открытыми портами 22, 80, 443"
    - "fail2ban защищает SSH с whitelist для автоматики"
    - "Существующие сервисы (OpenClaw) продолжают работать"
    - "Dockerfile собирает Next.js standalone из монорепо"
  artifacts:
    - path: "Dockerfile"
...
[DEFAULT]
# Whitelist: localhost. Add vps-ops-manager IP later via:
...
docker ps 2>/dev/null
```
Все команды должны вернуть успешный результат. Существующие контейнеры/сервисы в `docker ps` не изменились.
  </verify>
  <done>Docker Engine + Compose, Nginx, UFW (22/80/443), fail2ban установлены и работают. /home/deploy/maal/ создан. Существующие сервисы не затронуты.</done>
</task>
...
2. `docker --version` и `docker compose version` возвращают версии
3. `sudo nginx -t` проходит без ошибок
4. `ufw status` показывает порты 22, 80, 443 открытыми
5. `sudo fail2ban-client status sshd` показывает jail активным
6. `docker ps` показывает ВСЕ существующие контейнеры без изменений
7. Файлы Dockerfile, docker-compose.yml, .dockerignore существуют в корне монорепо
8. `grep "standalone" apps/web/next.config.js` находит настройку
</verification>

<success_criteria>
- VPS 89.208.106.208 доступен через SSH
- Docker Engine + Docker Compose работают
