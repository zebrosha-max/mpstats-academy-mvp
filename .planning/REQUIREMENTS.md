...
- [x] **TRACK-01**: Тип LearningPathSection расширен id `'custom'` и полем `addedAt` для ручных добавлений
- [x] **TRACK-02**: tRPC мутация `addToTrack` добавляет урок в секцию "Мои уроки", при отсутствии трека создаёт новый LearningPath
- [x] **TRACK-03**: tRPC мутация `removeFromTrack` удаляет урок из любой секции (custom или AI)
- [x] **TRACK-04**: tRPC мутация `rebuildTrack` перегенерирует AI-секции из последней диагностики, сохраняя "Мои уроки"
- [x] **TRACK-05**: Завершение диагностики сохраняет существующую custom-секцию (не перезаписывает)
- [x] **TRACK-06**: Кнопка "+" на LessonCard в режиме "Все курсы" — toggle "+"/"checkmark" для добавления в трек
- [x] **TRACK-07**: Кнопка "Убрать" в режиме "Мой трек" для удаления из любой секции
- [x] **TRACK-08**: Секция "Мои уроки" отображается первой (выше AI-секций) с фиолетовым стилем
- [x] **TRACK-09**: Кнопка "Перестроить трек" с диалогом подтверждения
- [x] **TRACK-10**: Toast-уведомления (sonner) при добавлении, удалении и перестройке трека
...
Requirements for Lesson Comments phase.

### Lesson Comments
- [x] **COMM-01**: Prisma-модель LessonComment с self-relation parentId для 1-уровневой вложенности и cascade delete
- [x] **COMM-02**: tRPC роутер comments с list (cursor-based пагинация 20 шт), create (валидация 1-1500 символов), delete (свои + admin)
- [x] **COMM-03**: Валидация вложенности: ответы только на корневые комментарии (parentId.parentId === null)
- [x] **COMM-04**: Компонент комментария показывает аватар (fallback инициалы), display name, относительную дату, контент и кнопку Ответить
- [x] **COMM-05**: На десктопе комментарии под AI-чатом в правой колонке, на мобилке — табы (AI-чат + Комментарии)
- [x] **COMM-06**: Optimistic updates при отправке и удалении комментариев с toast при ошибке
## Future Requirements
...
| COMM-04 | Phase 35 (Plan 02) | Complete |
| COMM-05 | Phase 35 (Plan 02) | Complete |
| COMM-06 | Phase 35 (Plan 02) | Complete |
...
- v2.0 requirements: 6 total, 0 planned
- Unmapped: 0

---
*Requirements defined: 2026-03-06*
*Last updated: 2026-03-26 after Phase 35 planning*
