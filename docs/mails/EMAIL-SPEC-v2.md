...
**Получатель:** юзер сменил email в `/profile`, нужно подтвердить новый адрес.
...
**Заголовок:** «Подтвердите новую почту»
**Тело:**
- `Вы запросили смену email на {{pa_new_email}} в Academy.`
- `Для подтверждения нажмите на кнопку ниже. Ссылка действительна 24 часа.`
- CTA-кнопка: «Подтвердить email» → `{{pa_confirm_url}}`
- Footer: `Если вы не запрашивали смену — проигнорируйте письмо.`
