...
**Tester Mila feedback batch — track UX + chat disclaimer. Задеплоено на прод (`ade7768`). Phase 55 vision chunking записана в roadmap (`7c15dc2`).**

**Контекст:** Мила (тестер) пожаловалась на 3 вещи:
1. В «Мой трек» выбирала курс «AI инструменты» через фильтр + кнопку «Перестроить трек» — ожидала что трек заполнится 87 уроками AI-курса, но осталось 38 как было. Toast «курс перестроен» появлялся, но контент не менялся.
2. «Задать вопрос по уроку» — ассистент молчит на любые вопросы. Гипотеза Милы: ждёт привязки доп.материалов (Phase 49).
3. UX-feedback: уроки по 3 минуты, не успеваешь переключаться.

**Диагностика:**
1. **Track rebuild ignores filters by design.** `learning.ts:1116` — `rebuildTrack` принимает 0 параметров, всегда регенерирует трек по последней диагностике через `generateSectionedPath`. Фильтры в UI применяются client-side только для отображения, не для пересборки. Тост-успех — формально корректный (трек реально пересобран), но не учитывает интент юзера. Это product-confusion, не баг.
2. **RAG молчит когда нет чанков для урока.** `generation.ts:186` — `searchChunks` ищет только по `content_chunk` (транскрипты аудио). Гипотеза Милы про материалы — мимо, RAG никогда к ним не подключался. Реальная причина в её случае: вопросы были про визуальный контент (что в ячейке D6, дай ссылку с экрана) — этого нет в транскрипте, спикер не озвучивал. LLM честно говорит «не знаю» по правилу «отвечай ТОЛЬКО на основе контекста».
3. UX 3-мин уроков — задача методологам (Мила/Катя), не код.

**Что задеплоено (`ade7768`):**
- **Бэк:** новая мутация `learning.addLessonsToTrack({ lessonIds[] })` (bulk до 500 уроков, та же custom-секция логика что в `addToTrack`).
- **`/learn` шапка трека:** кнопка `Перестроить трек` → `Перестроить по диагностике`. Диалог переписан — явно объясняет что это пересборка по последней диагностике + что фильтры выше она не учитывает + куда идти за «положи мне курс целиком».
- **`/learn` hint** под шапкой трека (всегда виден): «Фильтры скрывают уроки в текущем треке. Чтобы добавить новые — открой "Все курсы" и нажимай "+ В трек"».
- **`/learn` карточка курса в режиме «Все курсы»:** новая кнопка `+ В трек (N)` рядом с «Продолжить просмотр». N = сколько уроков курса ещё не в треке и не locked. Скрыта если все уже в треке. Под капотом — `addLessonsToTrack` одной транзакцией.
- **Чат урока (обе версии layout — desktop + mobile):** дисклеймер под полем ввода: «Я отвечаю на основе того, что говорит спикер в видео. На вопросы про то, что показано на экране (таблицы, ссылки, скриншоты), пока ответить не смогу — мы работаем над этим. По сложным вопросам напиши в комментарии под уроком — модератор подтянется.»

**Что НЕ менял:** логику `rebuildTrack` на бэке (она корректна для своей семантики), фильтры в треке (продолжают работать как display-фильтры), RAG retrieval (Phase 55 закроет).

**Phase 55: Vision Chunking RAG (commit `7c15dc2`)** — записано в `.planning/ROADMAP.md` как новый milestone v1.7 RAG Quality. Закрывает Милин паттерн «спрашиваю про то, что вижу на экране». Архитектура: ffmpeg scene-detection → VLM (GPT-4o-mini / Gemini Flash / Claude Haiku VL — выбор по PoC) + tesseract OCR → объединение → embedding в `content_chunk` с `source_type='frame'` + `frame_url` + `timecode` → mixed retrieval (5 transcript + 3 frame). 3 спринта с gates: PoC 1-2д → Pilot на курсе AI-инструменты 87 уроков 2д → Production 440 уроков 1д. Стоимость прогона ~$3-5 единоразово на GPT-4o-mini, ~$0.01-0.03 на новый урок. Дисклеймер в чате — временный пластырь до запуска. Открытые вопросы перед стартом: исходники видео (Kinescope download / `E:/Academy Courses`), VLM API аккаунт, кто составит контрольный датасет визуальных вопросов, privacy-промпт для VLM.

**Коммиты:** `ade7768` (track UX + chat disclaimer) → `7c15dc2` (Phase 55 roadmap entry).
